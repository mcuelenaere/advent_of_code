...
def drip(matrix: Matrix, origin: Coordinate):
    ...
    while len(queue) > 0:
        ...
        bounded_left_edge = tile_at(left_boundary - 1, pos[1]) not in ('.', '|') and tile_at(left_boundary, pos[1] + 1) not in ('.', '|')
        bounded_right_edge = tile_at(right_boundary + 1, pos[1]) not in ('.', '|') and tile_at(right_boundary, pos[1] + 1) not in ('.', '|')
