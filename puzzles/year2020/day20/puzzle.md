...
When looking for this pattern in the image, _the spaces can be anything_; only the `` # `` need to match. Also, you might need to rotate or flip your image before it's oriented correctly to find sea monsters. In the above image, _after flipping and rotating it_ to the appropriate orientation, there are _two_ sea monsters (marked with <code><em>O</em></code>):
